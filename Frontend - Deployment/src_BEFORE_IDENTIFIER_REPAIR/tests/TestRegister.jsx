...
const Assignedqualifying examsDropDown = ({
  item,
  isExpanded,
  setIsExpanded,
  setSelectedSubject,
  ...
  const fetchAssignedqualifying exams = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        console.error("Failed to fetch assigned qualifying exams:", response.status);
      }
    } catch (error) {
  ...
  const searchResults = assignedqualifying exams?.filter((subject) =>
  ...
  const handleDeleteSubject = async (subjectID) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setqualifying exams((prevqualifying exams) =>
          prevqualifying exams.filter((subject) => subject.subjectID !== subjectID),
        );
        setFilteredqualifying exams((prevqualifying exams) =>
          prevqualifying exams.filter((subject) => subject.subjectID !== subjectID),
        );
      } else {
  ...
  return (
    <div className="-mt-2">
      ...
      >
        <SideBarToolTip label="qualifying exams" isExpanded={isExpanded}>
      ...
      >
        ...
        >
          {subjectLoading ? (
            ...
            // Show filtered qualifying exams when searching
            filteredqualifying exams.map((subject) => (
              <li
              ...
              {yearLevelOptions.map((yearLevel) => (
                <li
                  ...
                  onClick={(e) => {
                    ...
                    setShowYearqualifying exams(true);
                    setOpenMenuID(null);
                  }}
                >
      ...
      {/* Floating qualifying exams Panel */}
          ...
          <>
            ...
            >
              ...
              >
                ...
                >
                  {(() => {
                    // Group assignedqualifying exams by yearLevel
                    const yearLevelGroups = assignedqualifying exams.reduce(
                      (acc, subject) => {
                    ...
                    return yearLevelGroups[selectedYearLevel]?.length > 0 ? (
                      ...
                      ).map(([programName, qualifying exams], index, array) => (
                        <div
                          key={programName}
                          onMouseDown={(e) => e.stopPropagation()}
                          onTouchStart={(e) => e.stopPropagation()}
                      ...
                      <div className="p-2 text-center text-sm text-gray-500">
                        No qualifying exams in this year level
                      </div>
                    );
                  })()}
                </div>
            ...
            >
              ...
              <div className="border-b border-gray-200 p-2">
                <div className="flex items-center justify-between">
                  ...
                  <button
                    onClick={() => setShowYearqualifying exams(false)}
                    className="cursor-pointer text-gray-500 hover:text-gray-700"
                  >
                    <i className="bx bx-x text-xl"></i>
                  </button>
...
export default Assignedqualifying examsDropDown;
