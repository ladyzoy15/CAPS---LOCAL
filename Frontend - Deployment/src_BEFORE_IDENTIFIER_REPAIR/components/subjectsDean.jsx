...
const SideBarDropDown = ({
  ...
  useEffect(() => {
    ...
    window.addEventListener("openqualifying examsidebar", openSidebar);
    return () => window.removeEventListener("openqualifying examsidebar", openSidebar);
  }, []);
  ...
  const fetchqualifying exams = async () => {
    ...
    try {
      ...
      if (!data.success) {
        throw new Error(data.message || "Failed to fetch qualifying exams");
      }
  ...
  const handleAddSubject = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        setqualifying exams((prevqualifying exams) => [...prevqualifying exams, newSubject]);
        setFilteredqualifying exams((prevqualifying exams) => [...prevqualifying exams, newSubject]);
  ...
  }, [isExpanded, qualifying exams]);

  useEffect(() => {
    if (!searchTerm.trim()) {
      setFilteredqualifying exams(qualifying exams);
    } else {
      const results = qualifying exams.filter(
        (subject) =>
          subject?.subjectName
            ?.toLowerCase()
              ...
              >
                ...
                {showProgramFilter && (
                  <div className="animate-dropdown animate-fadein absolute top-8 left-0 z-20 min-w-[150px] rounded border border-gray-300 bg-white p-1 shadow-lg">
                    ...
                    {Array.from(
                      new Set(qualifying exams.map((s) => s.programName)),
                    ).map((programName) => (
                      <div
                        key={programName}
                        className="flex cursor-pointer items-center gap-2 rounded-sm py-2 text-sm hover:bg-gray-100"
            ...
            <div className="custom-scrollbar flex-1 overflow-y-auto pb-16 sm:pb-4">
              <ul className="w-full">
                {subjectLoading ? (
                  ...
                  Object.entries(
                    qualifying exams.reduce((acc, subject) => {
                      const program = subject.programName || "Unassigned";
                      if (!acc[program]) acc[program] = [];
                      acc[program].push(subject);
                      return acc;
