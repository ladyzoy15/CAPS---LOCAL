...
import PracticeExamConfig from "./qualifying examsettingsDean";
...
const SubjectCard = ({
  ...
  // directly on "Review Question" (formerly "Subject", index 1).
  const tabs = [
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/qualifying exams/${subjectID}/update`,
        {
          method: "PUT",
          headers: {
      ...
      if (response.ok) {
        ...
        if (refreshqualifying exams) refreshqualifying exams();
        window.dispatchEvent(new Event("refreshqualifying examsList"));
        ...
        switch (response.status) {
          case 401:
            ...
            break;
          case 403:
            showToast("You are not authorized to modify qualifying exams.", "error");
            break;
          case 404:
  ...
  const handleDeleteSubject = async (subjectID) => {
    ...
    try {
      const response = await fetch(
        `${import.meta.env.VITE_API_BASE_URL}/qualifying exams/${subjectID}/delete`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
