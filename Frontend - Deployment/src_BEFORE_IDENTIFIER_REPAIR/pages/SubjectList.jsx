...
function SubjectList() {
  ...
  const basePath =
    Number(effectiveRoleId) === 2
      ? "/faculty/qualifying exams"
      : Number(effectiveRoleId) === 3
        ? "/program-chair/qualifying exams"
        : Number(effectiveRoleId) === 5
          ? "/asso-dean/qualifying exams"
          : "/dean/qualifying exams";
  ...
  useEffect(() => {
    if (subjectFromState && subjectFromState.subjectID) {
      ...
      const fetchSubject = async () => {
        ...
        try {
          const response = await fetch(`${apiUrl}/qualifying exams/${subjectID}`, {
            headers: {
  ...
  useEffect(() => {
    ...
    if (subjectFromState && subjectFromState.subjectID) {
      ...
      const contentPath =
        Number(effectiveRoleId) === 2
          ? "/faculty/qualifying exams/content"
          : Number(effectiveRoleId) === 3
            ? "/program-chair/qualifying exams/content"
            : Number(effectiveRoleId) === 5
              ? "/asso-dean/qualifying exams/content"
              : "/dean/qualifying exams/content";
  ...
  const [availablequalifying exams, setAvailablequalifying exams] = useState([]);
  const [loadingAvailablequalifying exams, setLoadingAvailablequalifying exams] =
    useState(false);
...
// Close kebab menu when clicking outside or scrolling
  ...
  // Get unique program names for sidebar buttons — derived from actual qualifying exams
  // so only programs that have at least one subject are shown.
  const uniquePrograms = Array.from(
    new Set(qualifying exams.map((s) => s.programName).filter(Boolean)),
  ).sort();

  // Group filtered qualifying exams by program
  ...
  // Sort programs and their qualifying exams
  ...
  // Filter qualifying exams based on selected program, search term, and year level
  useEffect(() => {
    let filtered = qualifying exams;
  ...
  }, [selectedProgramFilter, selectedYearLevelFilter, qualifying exams, searchTerm]);

  // Fetch qualifying exams
  useEffect(() => {
    if (effectiveRoleId) {
  ...
  useEffect(() => {
    const handleRefresh = () => fetchqualifying exams();
  ...
  const fetchqualifying exams = async () => {
    ...
    try {
      // If user is faculty, fetch assigned qualifying exams instead of all qualifying exams
      const endpoint =
        Number(effectiveRoleId) === 2
          ? `${apiUrl}/faculty/my-qualifying exams`
          : `${apiUrl}/qualifying exams`;
      ...
      // Faculty endpoint returns data.qualifying exams directly, admin endpoint returns data.success
      const qualifying examsArray =
        Number(effectiveRoleId) === 2
          ? Array.isArray(data.qualifying exams)
            ? data.qualifying exams
            : []
          : data.success && Array.isArray(data.qualifying exams)
            ? data.qualifying exams
            : [];

      if (!Array.isArray(qualifying examsArray)) {
        console.error("Unexpected qualifying exams data format:", data);
        return;
      }

      const sortedSubjects = [...qualifying examsArray].sort((a, b) => {
      ...
      setqualifying exams(sortedSubjects);
  ...
  const handleAddSubject = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/add-qualifying exams`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/qualifying exams/${editingSubject}/update`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteSubject = async () => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/qualifying exams/${subjectToDelete.subjectID}/delete`,
        {
          method: "DELETE",
          headers: {
            "Content-Type": "application/json",
  ...
  const fetchAvailablequalifying exams = async () => {
    const token = sessionStorage.getItem("token");
    setLoadingAvailablequalifying exams(true);
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          console.error(
            "Forbidden: Only Faculty can access available qualifying exams.",
          );
          return;
        }
        throw new Error(`HTTP error! Status: ${response.status}`);
      ...
      if (!Array.isArray(data.qualifying exams)) {
        console.error("Unexpected available qualifying exams data format:", data);
        return;
      }

      // Sort qualifying exams alphabetically by subjectCode
      const sortedSubjects = [...data.qualifying exams].sort((a, b) =>
        a.subjectCode.localeCompare(b.subjectCode),
      );

      setAvailablequalifying exams(sortedSubjects);
    } catch (error) {
      console.error("Error fetching available qualifying exams:", error);
    } finally {
      setLoadingAvailablequalifying exams(false);
    }
  };
  ...
  // Get unassigned qualifying exams (available qualifying exams that are not already assigned)
  const unassignedqualifying exams = availablequalifying exams.filter(
    (subject) =>
      !qualifying exams.some((assigned) => assigned.subjectID === subject.subjectID),
  );
          ...
          <div className="scrollbar-hide mt-10 ml-0 flex min-h-screen flex-1 flex-col gap-6 overflow-y-auto pb-0 [-ms-overflow-style:none] [scrollbar-width:none] lg:mt-0 [&::-webkit-scrollbar]:hidden">
            <div className="min-w-0 space-y-4 px-4 pt-4 md:px-6 md:pt-6">
              <SearchBar
                ...
                placeholder="Search qualifying exams..."
                mobileCollapsible
                    ...
                    <button
                      ...
                      onClick={() => {
                        if (Number(effectiveRoleId) === 2) {
                          ...
                          fetchAvailablequalifying exams();
                        } else {
            ...
            {/* qualifying exams List */}
            <div className="md:px-4">
              {subjectLoading ? (
                <div className="outfit-400 flex h-64 items-center justify-center">
                  <div className="text-center">
                    <div className="loader mx-auto mb-2"></div>
                    <p className="text-[14px] text-gray-600">Loading qualifying exams...</p>
                  </div>
                </div>
              ) : networkError ? (
                ...
                <>
                  <div className="space-y-8">
                    {sortedPrograms.map((programName) => {
                      const programqualifying exams = groupedSubjects[programName];
                      const subjectCount = programqualifying exams.length;
                      ...
                      return (
                        ...
                        >
                          ...
                          {/* qualifying exams Grid for this Program */}
                          <div className="mt-4 grid grid-cols-1 space-y-2 space-x-2 px-2 md:grid-cols-[repeat(auto-fill,minmax(20rem,20rem))] md:gap-2 lg:px-4">
                            {programqualifying exams.map((subject) => {
                  ...
                  <div className="px-5 py-4">
                    <div className="mt-2 flex w-full items-center gap-2">
                      ...
                      <button
                        ...
                        onClick={fetchAvailablequalifying exams}
                      >
                    ...
                    <div className="mb-3">
                      <ul className="max-h-[200px] overflow-y-auto">
                        {loadingAvailablequalifying exams ? (
