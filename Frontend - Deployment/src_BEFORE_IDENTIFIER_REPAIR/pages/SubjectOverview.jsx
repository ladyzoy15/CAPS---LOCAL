    ...
    const fetchAllData = async () => {
      ...
      try {
        ...
        const [questionsRes, resultsRes] = await Promise.all([
          fetch(`${apiUrl}/qualifying exams/${subject.subjectID}/questions`, {
            method: "GET",
            headers: {
  ...
  const handleSaveEdit = async () => {
    ...
    try {
      const response = await fetch(
        `${apiUrl}/qualifying exams/${subject.subjectID}/update`,
        {
          method: "PUT",
          headers: {
            "Content-Type": "application/json",
  ...
  const handleDeleteSubject = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const user = JSON.parse(sessionStorage.getItem("user"));
        const roleID = user?.roleID ?? user?.roleId;
        let path = "/dean/qualifying exams";
        if (Number(roleID) === 2) path = "/faculty/qualifying exams";
        else if (Number(roleID) === 3) path = "/program-chair/qualifying exams";
        navigate(path);
      } else {
          ...
          <div className="relative mt-4 flex items-start justify-start sm:items-end sm:justify-end">
            <button
              ...
              onClick={() => {
                const user = JSON.parse(sessionStorage.getItem("user"));
                const roleID = user?.roleID ?? user?.roleId;
                let path = "/dean/qualifying exams?subject_id=" + subject.subjectID;
                if (Number(roleID) === 2)
                  path = "/faculty/qualifying exams?subject_id=" + subject.subjectID;
                else if (Number(roleID) === 3)
                  path =
                    "/program-chair/qualifying exams?subject_id=" + subject.subjectID;
                navigate(path, { state: { subject } });
                // Dispatch a custom event to close the sidebar/subject list
                window.dispatchEvent(new Event("closequalifying examsidebar"));
              }}
            >
