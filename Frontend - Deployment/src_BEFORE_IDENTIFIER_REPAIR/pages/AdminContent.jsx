...
// Main admin dashboard component for managing questions and qualifying exams
const AdminContent = () => {
  ...
  useEffect(() => {
    if (selectedSubject && selectedSubject.subjectID) {
      const fetchqualifying examsettings = async () => {
        const token = sessionStorage.getItem("token");
        try {
          // Fetch QE status
          const qeResponse = await fetch(
            `${apiUrl}/qualifying exams/${selectedSubject.subjectID}/exam-questions-status`,
      ...
      fetchqualifying examsettings();
    }
  }, [selectedSubject, apiUrl]);

  // Re-fetch settings when qualifying exams list is refreshed (e.g. after saving settings)
  useEffect(() => {
    const handleRefresh = () => {
      if (selectedSubject && selectedSubject.subjectID) {
  ...
  const fetchQuestions = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${apiUrl}/qualifying exams/${selectedSubject.subjectID}/questions`,
        {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
