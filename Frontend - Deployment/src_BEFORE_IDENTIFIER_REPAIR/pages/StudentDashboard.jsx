...
const StudentDashboard = () => {
  ...
  const [qualifying examsLoading, setqualifying examsLoading] = useState(true);
  ...
  useEffect(() => {
    const fetchqualifying exams = async () => {
      try {
        const res = await fetch(`${apiUrl}/student/practice-qualifying exams`, {
          headers: {
        ...
        if (data.data) {
          setqualifying exams(data.data.filter(isAvailablePracticeSubject));
        }
      } catch (err) {
        console.error("Error fetching qualifying exams:", err);
      } finally {
        setqualifying examsLoading(false);
      }
    };
  ...
  const filtered = qualifying exams.filter((s) => {
          ...
          <div className="mx-auto mt-6 max-w-2xl md:mt-8">
            <div className="flex items-center gap-2 rounded-full">
              <div className="flex flex-1 items-center overflow-hidden rounded-full border border-gray-200 bg-white shadow-[0_4px_10px_rgba(249,115,22,0.18)]">
                ...
                <input
                  type="text"
                  placeholder="Search for qualifying exams.."
                  value={searchQuery}
                  maxLength={50}
        ...
        <div className="mx-auto mt-5 max-w-3xl px-4 pb-20 md:mt-6 md:px-0 md:pb-28">
          {qualifying examsLoading ? (
            ...
            <div className="outfit-400 rounded-xl border border-dashed border-gray-200 py-12 text-center text-[14px] text-gray-400">
              {searchQuery
                ? `No qualifying exams found for "${searchQuery}"`
