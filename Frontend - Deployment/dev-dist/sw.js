...
if (!self.define) {
  ...
  // Used for `eval` and `importScripts` where we can't get script URL
  // by other means.
  // In both cases, it's safe to use a global var because those functions
  // are synchronous.
  let nextDefineUri;
  ...
  self.define = (depsNames, factory) => {
    ...
    const require = (depUri) =>
      singleRequire(depUri, uri);
...
define(["./workbox-5a5d9309"], function (workbox) {
  ...
  // Handle navigation requests through index.html.
  workbox.registerRoute(
    new workbox.NavigationRoute(
      workbox.createHandlerBoundToURL(
        "index.html"
      ),
      {
        allowlist: [/^\/$/]
