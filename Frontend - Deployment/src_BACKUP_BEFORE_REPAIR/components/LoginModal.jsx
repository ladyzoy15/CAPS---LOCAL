...
export default function LoginModal({
  ...
  const handleLogin = async (e) => {
    ...
    try {
      ...
      // empty bodies, plain-text errors) so JSON.parse doesn't throw
      // and get swallowed by the generic catch block below.
      // We clone the response first so we can read the raw text
      // separately if json() parsing fails (a body can only be
      // read once otherwise).
      let data;
      try {
        ...
        console.error("LOGIN: raw response body was:", raw);
        console.error("LOGIN: response status was:", response.status, response.statusText);
        showToast("Unexpected server response. Please try again later.", "error");
        return;
      }

      // Log the raw parsed response so you can inspect the actual shape
      ...
      // Defensive checks — if the backend's success response doesn't
      // actually include `user` / `roleID` in the shape we expect,
      // show a clear message instead of silently crashing into the
      // catch block and showing the generic "something went wrong" toast.
      if (!data || !data.user) {
      ...
      if (dashboardPath) {
        // Close the modal BEFORE navigating — this modal is likely mounted
        // outside of <Routes> (controlled by isOpen from a parent), so
        // navigate() alone will NOT unmount it. Without calling onClose(),
        // the modal stays visible on top of the new route, making it look
        // like login "did nothing" even though it actually succeeded and
        // navigated in the background.
        onClose();
      ...
      // Log the actual error instead of swallowing it silently, so
      // future issues are visible in the console instead of just
