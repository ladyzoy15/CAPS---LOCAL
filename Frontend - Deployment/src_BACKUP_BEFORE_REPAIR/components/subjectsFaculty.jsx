...
import qualifying examsIcon from "/src/assets/symbols/qualifying exams.svg";
import qualifying examsIconH from "/src/assets/symbols/qualifying examshover.svg";
...
const SideBarDropDown = ({
  ...
  const [filteredqualifying exams, setFilteredqualifying exams] = useState([]);
  ...
  const [showYearqualifying exams, setShowYearqualifying exams] = useState(false);
  const [openKebabMenu, setOpenKebabMenu] = useState(null);
  const [showSearch, setShowSearch] = useState(false);
  const [assignedqualifying exams, setAssignedqualifying exams] = useState([]);

  const dropdownRef = useRef(null);
  const [showProgramFilter, setShowProgramFilter] = useState(false);
  const [selectedProgramFilter, setSelectedProgramFilter] = useState("All");
  const programFilterRef = useRef(null);
  const unassignedqualifying exams = qualifying exams.filter(
    (subject) =>
      !assignedqualifying exams.some(
  ...
  useEffect(() => {
    const handleResize = () => {
      ...
      if (wasSmallScreen && !isNowSmallScreen && isOpen) {
        ...
        setSearchTerm("");
        setFilteredqualifying exams(assignedqualifying exams);
        setShowYearqualifying exams(false);
      }
    ...
    const handleCloseSidebar = () => {
      ...
      setShowAddModal(false);
      setSearchTerm("");
      setFilteredqualifying exams(qualifying exams);
      setIsExpanded(false);
      setShowYearqualifying exams(false);
    };
    window.addEventListener("closequalifying examsidebar", handleCloseSidebar);
    return () => {
      window.removeEventListener("resize", handleResize);
      window.removeEventListener("closequalifying examsidebar", handleCloseSidebar);
    };
  }, [
    isMobile,
    isOpen,
  ...
  const fetchqualifying exams = async () => {
    ...
    try {
      const response = await fetch(`${apiUrl}/faculty/availablequalifying exams`, {
        method: "GET",
        headers: {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          console.error("Forbidden: Only Deans can access qualifying exams.");
          return;
        }
      ...
      if (!Array.isArray(data.qualifying exams)) {
        console.error("Unexpected qualifying exams data format:", data);
        return;
      }

      // Sort qualifying exams alphabetically by subjectCode
      const sortedqualifying exams = [...data.qualifying exams].sort((a, b) =>
        a.subjectCode.localeCompare(b.subjectCode),
      );

      setqualifying exams(sortedqualifying exams);
    } catch (error) {
      console.error("Error fetching qualifying exams:", error);
    } finally {
      setLoading(false);
    }
  };

  const fetchAssignedqualifying exams = async () => {
    const token = sessionStorage.getItem("token");
    setSubjectLoading(true);
    try {
      const response = await fetch(`${apiUrl}/faculty/my-qualifying exams`, {
        method: "GET",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      if (response.ok) {
        const data = await response.json();
        console.log("Fetched assigned qualifying exams:", data); // Debug log

        if (!Array.isArray(data.qualifying exams)) {
          console.error("Unexpected assigned qualifying exams data format:", data);
          return;
        }
        ...
        console.log("Setting assigned qualifying exams:", sortedqualifying exams); // Debug log
        setAssignedqualifying exams(sortedqualifying exams);
        setFilteredqualifying exams(sortedqualifying exams);
      } else {
        console.error(
          "Failed to fetch assigned qualifying exams:",
          response.status,
          response.statusText,
        );
      }
    } catch (error) {
      console.error("Error fetching assigned qualifying exams:", error);
    } finally {
  ...
  }, [isExpanded, assignedqualifying exams]);

  useEffect(() => {
    if (!searchTerm.trim()) {
      setFilteredqualifying exams(assignedqualifying exams);
    } else {
      const results = assignedqualifying exams.filter(
        (subject) =>
          subject?.subjectName
            ?.toLowerCase()
      ...
      setFilteredqualifying exams(results);
    }
  }, [searchTerm, assignedqualifying exams]);
  ...
  const handleAssignSubject = async (subject) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        await fetchAssignedqualifying exams();
  ...
  // Group qualifying exams by year level
  const yearLevelGroups = filteredqualifying exams.reduce((acc, subject) => {
  ...
  }, [isOpen, assignedqualifying exams]);

  // Expose refreshqualifying exams function
  const handleRefreshqualifying exams = () => {
    fetchAssignedqualifying exams();
  };

  useEffect(() => {
    const handleRefresh = () => fetchAssignedqualifying exams();
    window.addEventListener("refreshqualifying examsList", handleRefresh);
    return () =>
      window.removeEventListener("refreshqualifying examsList", handleRefresh);
  }, []);
    ...
    <div className="">
      ...
      >
        ...
        <div className="ml-3 flex items-center gap-3">
          <img
            src={isSubjectFocused ? qualifying examsIconH : qualifying examsIcon}
            alt="qualifying exams"
      ...
      {(isExpanded || isOpen) && (
        <>
          ...
          >
                  ...
                  <div className="w-full" ref={dropdownRef}>
                    <div className="flex items-center justify-between gap-1">
                      <button
                        ...
                        onClick={() => setShowYearqualifying exams((prev) => !prev)}
                      >
                        <span className="flex items-center gap-2 text-nowrap">
                          <i className="bx bx-list-ul text-2xl text-gray-500"></i>
                          {selectedYearLevel
                            ? `${selectedYearLevel}${selectedYearLevel === "1" ? "st" : selectedYearLevel === "2" ? "nd" : selectedYearLevel === "3" ? "rd" : "th"} Year qualifying exams`
                            : "All Year Level"}
                        </span>
                        <i
                          className={`bx bx-chevron-down ml-1 text-2xl text-gray-500 transition-transform duration-200 ease-in-out ${
                            showYearqualifying exams ? "rotate-180" : ""
                          }`}
                        />
                      </button>
                    </div>
                      ...
                      <ul className="animate-dropdown animate-fadein absolute left-0 z-10 mx-2 mt-1 w-[calc(100%-16px)] rounded border border-gray-300 bg-white p-1 shadow-lg">
                        ...
                        {yearLevelOptions.map((yearLevel) => (
                          ...
                          >
                            ...
                            {`${yearLevel}${yearLevel === "1" ? "st" : yearLevel === "2" ? "nd" : yearLevel === "3" ? "rd" : "th"} Year qualifying exams`}
                          </li>
                        ))}
                      </ul>
                    )}
              ...
              >
                ...
                {showProgramFilter && (
                  <div className="animate-dropdown animate-fadein absolute top-8 left-0 z-20 min-w-[150px] rounded border border-gray-300 bg-white p-1 shadow-lg">
                    ...
                    {Array.from(
                      new Set(assignedqualifying exams.map((s) => s.programName)),
                    ).map((programName) => (
                      <div
                        key={programName}
                        className="flex cursor-pointer items-center gap-2 rounded-sm py-2 text-sm hover:bg-gray-100"
            ...
            {/* qualifying exams List (filtered by search or year level) */}
            <div className="custom-scrollbar flex-1 overflow-y-auto pb-16 sm:pb-4">
              <ul className="w-full">
                {subjectLoading ? (
                ...
                ) : assignedqualifying exams.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-10">
                    <img
                      src={emptyImage}
                      alt="No qualifying exams available"
                      className="h-32 w-32 opacity-80"
                    />
                    <span className="text-[14px] font-semibold text-gray-500">
                      No qualifying exams Available
                    </span>
                  </div>
                ) : searchTerm.trim() && filteredqualifying exams.length === 0 ? (
                  <div className="flex flex-col items-center justify-center py-10">
                    <img
                      src={notFoundImage}
                  ...
                  // Group qualifying exams by program for search results
                  Object.entries(
                    filteredqualifying exams.reduce((acc, subject) => {
                    ...
                    .map(([programName, qualifying exams]) => (
                      <div key={programName}>
                        {selectedProgramFilter === "All" && (
                  ...
                  // Show all assigned qualifying exams when "All qualifying exams" is selected
                  Object.entries(
                    filteredqualifying exams.reduce((acc, subject) => {
                      const program = subject.programName || "Unassigned";
                      if (!acc[program]) acc[program] = [];
                  ...
                  yearLevelGroups[selectedYearLevel]?.length > 0 ? (
                    // Group qualifying exams by program for year level results
                    Object.entries(
                  ...
                  <div className="p-2 text-center text-sm text-gray-500">
                    Select a year level or search to view qualifying exams
                  </div>
                )}
              </ul>
            </div>
            ...
            <div className="custom-scrollbar flex-1 overflow-y-auto pb-16 sm:pb-4">
              <ul className="w-full">
                {subjectLoading ? (
                  ...
                  // Show all qualifying exams when "All qualifying exams" is selected
                  Object.entries(
                    filteredqualifying exams.reduce((acc, subject) => {
                      const program = subject.programName || "Unassigned";
                      if (!acc[program]) acc[program] = [];
          ...
          <div className="animate-fade-in-up border-color relative mx-auto w-full max-w-md rounded-b-md border border-t-0 bg-white p-2 sm:px-4">
            ...
            <div className="mb-3">
              <ul className="max-h-[200px] overflow-y-auto" ref={listRef}>
                {loading ? (
                ...
                ) : unassignedqualifying exams.length > 0 ? (
                  [...unassignedqualifying exams]
                    .filter((subject) =>
                  ...
                  <li className="p-2 text-center text-[14px] text-[rgb(168,168,168)]">
                    All qualifying exams already assigned.
                  </li>
                )}
              </ul>
            </div>
