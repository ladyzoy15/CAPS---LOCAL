...
function Programqualifying examselector({
  qualifying exams,
  ...
  const [qualifying examsearch, setqualifying examsearch] = useState("");

  // Group qualifying exams by program
  const programMap = React.useMemo(() => {
    const map = {};
    qualifying exams.forEach((s) => {
  ...
  const programqualifying exams = selectedProgram
    ? programMap[selectedProgram] || []
    : [];
  const filteredqualifying exams = qualifying examsearch.trim()
    ? programqualifying exams.filter(
        (s) =>
          s.subjectName?.toLowerCase().includes(qualifying examsearch.toLowerCase()) ||
          s.subjectCode?.toLowerCase().includes(qualifying examsearch.toLowerCase()),
      )
    : programqualifying exams;

  if (isqualifying examsLoading) {
    return (
    ...
    return (
      <div className="space-y-3">
        ...
        <div className="relative">
          ...
          <input
            type="text"
            value={qualifying examsearch}
            onChange={(e) => setqualifying examsearch(e.target.value)}
        ...
        <div className="max-h-56 space-y-1.5 overflow-y-auto pr-1">
          {filteredqualifying exams.length === 0 ? (
            <p className="py-4 text-center text-sm text-gray-400">
              No qualifying exams found.
            </p>
          ) : (
            filteredqualifying exams.map((s) => {
  ...
  useEffect(() => {
    const fetchqualifying exams = async () => {
      ...
      try {
        ...
        // qualifying exams + General Education (GE); dean/associate-dean get all. (Using
        // /qualifying exams/all here previously let faculty/chair see every program.)
        const response = await fetch(
        ...
        if (!response.ok) {
          ...
          let errorMessage = "Failed to load qualifying exams.";
          try {
            const errorData = await response.json();
            errorMessage =
        ...
        // Sort qualifying exams similar to your pattern
        ...
        setError(err.message || "Failed to load qualifying exams. Please try again.");
                ...
                <div className="flex flex-1 flex-col">
                  ...
                  <div className="outfit-400 flex-1 overflow-y-auto px-6 py-5">
                    <form id="quiz-create-form" onSubmit={handleSubmit}>
                      ...
                      {currentStep === 3 && formData.quiz_type_id === "1" && (
                        <Programqualifying examselector
                          qualifying exams={qualifying exams}
                          isqualifying examsLoading={isqualifying examsLoading}
