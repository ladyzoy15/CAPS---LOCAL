...
const PracticeChoicesForm = ({ questionID, onComplete, onCancel }) => {
      ...
      <div className="flex-1">
        <div className="border-color relative mx-auto w-full max-w-3xl rounded-b border border-t-0 bg-white p-4 shadow-lg sm:px-4">
          <div className="space-y-3 px-3 py-2">
            ...
            {toast.message && (
              <div
                className={`fixed bottom-5 left-5 z-56 rounded px-4 py-2 text-sm text-white shadow-lg ${toast.type === "success" ? "bg-green-500" : "bg-red-500"} ${toast.show ? "opacity-100" : "opacity-0"} transition-opacity duration-500 ease-in-out`}
              >
                {toast.message}
              </div>
            )}
