    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          let errorMessage = "You do not have permission to access subjects.";
          try {
        ...
        // Handle 404 No subjects found
        if (response.status === 404) {
          let errorMessage = "No subjects found.";
          try {
            const errorData = await response.json();
            errorMessage = errorData?.message || errorMessage;
            // Set empty array if no subjects found (not necessarily an error)
            if (errorData?.subjects && Array.isArray(errorData.subjects)) {
              setSubjects([]);
              setIsSubjectsLoading(false);
              return;
            }
