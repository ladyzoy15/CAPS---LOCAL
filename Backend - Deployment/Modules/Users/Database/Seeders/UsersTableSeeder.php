<?php

namespace Modules\Users\Database\Seeders;

use Illuminate\Database\Seeder;
use Illuminate\Support\Facades\DB;
use Illuminate\Support\Facades\Hash;

class UsersTableSeeder extends Seeder
{
    public function run()
    {
        try {
            // Retrieve the status ID for registered users; fallback to 4 if not found
            $status = DB::table('statuses')->where('name', 'registered')->first();
            $registeredStatusId = $status ? $status->id : 4;

            $users = [
                // Main Campus Accounts
                [
                    'userCode' => '10-A-00000',
                    'firstName' => 'MAIN - CE',
                    'lastName' => 'Chair',
                    'email' => 'ce.chair@university.edu',
                    'roleID' => 3,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 3,
                ],
                [
                    'userCode' => '10-A-00001',
                    'firstName' => 'MAIN - CPE',
                    'lastName' => 'Chair',
                    'email' => 'cpe.chair@university.edu',
                    'roleID' => 3,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 1,
                ],
                [
                    'userCode' => '10-A-00010',
                    'firstName' => 'MAIN - EE',
                    'lastName' => 'Chair',
                    'email' => 'ee.chair@university.edu',
                    'roleID' => 3,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 2,
                ],
                [
                    'userCode' => '10-A-00011',
                    'firstName' => 'MAIN - ECE',
                    'lastName' => 'Chair',
                    'email' => 'ece.chair@university.edu',
                    'roleID' => 3,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 4,
                ],

                // Faculty
                [
                    'userCode' => '10-A-00100',
                    'firstName' => 'MAIN - CE',
                    'lastName' => 'Faculty',
                    'email' => 'ce.faculty@university.edu',
                    'roleID' => 2,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 3,
                ],
                [
                    'userCode' => '10-A-00101',
                    'firstName' => 'CPE',
                    'lastName' => 'Faculty',
                    'email' => 'cpe.faculty@university.edu',
                    'roleID' => 2,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 1,
                ],
                [
                    'userCode' => '10-A-00110',
                    'firstName' => 'MAIN - EE',
                    'lastName' => 'Faculty',
                    'email' => 'ee.faculty@university.edu',
                    'roleID' => 2,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 2,
                ],
                [
                    'userCode' => '10-A-00111',
                    'firstName' => 'MAIN - ECE',
                    'lastName' => 'Faculty',
                    'email' => 'ece.faculty@university.edu',
                    'roleID' => 2,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 4,
                ],

                // Students
                [
                    'userCode' => '10-A-01000',
                    'firstName' => 'MAIN - CE',
                    'lastName' => 'Student',
                    'email' => 'ce.student@university.edu',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 3,
                ],
                [
                    'userCode' => '10-A-01001',
                    'firstName' => 'MAIN - CPE',
                    'lastName' => 'Student',
                    'email' => 'cpe.student@university.edu',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 1,
                ],
                [
                    'userCode' => '10-A-01010',
                    'firstName' => 'MAIN - EE',
                    'lastName' => 'Student',
                    'email' => 'ee.student@university.edu',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 2,
                ],
                [
                    'userCode' => '10-A-01011',
                    'firstName' => 'MAIN - ECE',
                    'lastName' => 'Student',
                    'email' => 'ece.student@university.edu',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 4,
                ],

                // Test Accounts
                [
                    'userCode' => '10-A-01110',
                    'firstName' => 'Testoooo',
                    'lastName' => 'Teroonn',
                    'email' => 'testtTer@gmail.com',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 1,
                ],
                [
                    'userCode' => '10-A-01120',
                    'firstName' => 'Testtt',
                    'lastName' => 'Teeerr',
                    'email' => 'TeessttTeerr@gmail.com',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 1,
                ],
                [
                    'userCode' => '10-A-01230',
                    'firstName' => 'Test',
                    'lastName' => 'Ter',
                    'email' => 'testTer@gmail.com',
                    'roleID' => 1,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 1,
                ],

                // Admin/Deans
                [
                    'userCode' => '10-A-02087',
                    'firstName' => 'Associate',
                    'lastName' => 'Dean',
                    'email' => 'associate.dean@university.edu',
                    'roleID' => 5,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 4,
                ],
                [
                    'userCode' => '10-A-12345',
                    'firstName' => 'MAIN - Dean',
                    'lastName' => 'Account',
                    'email' => 'dean.account@university.edu',
                    'roleID' => 4,
                    'campusID' => 1,
                    'isActive' => 1,
                    'programID' => 4,
                ],
                // Katipunan  Campus Accounts
                [
                    'userCode' => 'KT-A-00000',
                    'firstName' => 'Katipunan Associate Dean',
                    'lastName' => 'Account',
                    'email' => 'KTASSODEAN@university.edu',
                    'roleID' => 5,
                    'campusID' => 2,
                    'isActive' => 1,
                    'programID' => 5,
                ],
                [
                    'userCode' => 'KT-A-00001',
                    'firstName' => 'Katipunan  Chair',
                    'lastName' => 'Account',
                    'email' => 'KTACHAIR@university.edu',
                    'roleID' => 3,
                    'campusID' => 2,
                    'isActive' => 1,
                    'programID' => 5,
                ],
                [
                    'userCode' => 'KT-A-00010',
                    'firstName' => 'Katipunan Faculty',
                    'lastName' => 'Account',
                    'email' => 'KTFACULTY@university.edu',
                    'roleID' => 2,
                    'campusID' => 2,
                    'isActive' => 1,
                    'programID' => 5,
                ],
                //Tampilisan Campus Acccounts
                [
                    'userCode' => 'TC-A-00000',
                    'firstName' => 'Tampilisan Associate Dean',
                    'lastName' => 'Account',
                    'email' => 'TCASSODEAN@university.edu',
                    'roleID' => 5,
                    'campusID' => 3,
                    'isActive' => 1,
                    'programID' => 5,
                ],
                [
                    'userCode' => 'TC-A-00001',
                    'firstName' => 'Tampilisan Chair',
                    'lastName' => 'Account',
                    'email' => 'TCACHAIR@university.edu',
                    'roleID' => 3,
                    'campusID' => 3,
                    'isActive' => 1,
                    'programID' => 5,
                ],
                [
                    'userCode' => 'TC-A-00010',
                    'firstName' => 'Tampilisan Faculty',
                    'lastName' => 'Account',
                    'email' => 'TCFACULTY@university.edu',
                    'roleID' => 2,
                    'campusID' => 3,
                    'isActive' => 1,
                    'programID' => 5,
                ],
            ];

            foreach ($users as $userData) {
                DB::table('users')->updateOrInsert(
                    ['userCode' => $userData['userCode']], // Unique identifier
                    array_merge($userData, [
                        'password' => Hash::make('password123'), // Default password
                        'status_id' => $registeredStatusId,
                        'created_at' => now(),
                        'updated_at' => now(),
                    ])
                );
            }

            $this->command->info('Users table seeded successfully.');

        } catch (\Exception $e) {
            $this->command->error('Failed to seed users: ' . $e->getMessage());
        }
    }
}
